...
function CustomColumn({ colIdx, colName, pageIdx }: { colIdx: number; colName: string; pageIdx: number }) {
  ...
  const handleMobileAddClick = () => {
    if (isMobile && cards.length < MAX_CUSTOM_CARDS) {
      const firstCard = ALL_CARDS[0];
      if (firstCard) {
        placeCard(pageIdx, String(colIdx), { cardId: firstCard.id, catId: firstCard.categoryId });
      }
    }
  };
    ...
    <div className="flex flex-col border-r border-r-weekly-border last:border-r-0 min-w-0 overflow-hidden">
      <div className="bg-weekly-head-bg border-b border-b-weekly-border px-2 py-3 text-center shrink-0">
        <input
          ...
          className="w-full text-center border-none bg-transparent text-[14px] italic text-weekly-head-text tracking-wide outline-none hover:bg-white/60 focus:bg-white focus:shadow-[inset_0_0_0_1.5px_#7A8F5E] rounded-sm px-1 py-0.5 font-medium"
        />
      </div>
      <div
        ref={setNodeRef}
        ...
        className={`flex-1 flex flex-col gap-1 p-1 justify-center transition-colors duration-150
          ${isOver ? "bg-weekly-hover" : "bg-weekly-body-bg"}
          ${isMobile && cards.length < MAX_CUSTOM_CARDS ? "cursor-pointer" : ""}
        `}
      >
        {cards.map((cardRef, idx) => {
          const card = ALL_CARDS.find((c) => c.id === cardRef.cardId);
          ...
          return (
            <div key={idx} className="bg-white border border-[#E0E5D5] flex flex-col relative group flex-1 min-h-0 overflow-hidden">
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  removeCard(pageIdx, String(colIdx), idx);
                }}
                className="absolute top-1 right-1 w-[15px] h-[15px] bg-white/90 border border-[#DDD] rounded-full hidden group-hover:flex items-center justify-center cursor-pointer text-[11px] text-[#888] leading-none z-[3] hover:bg-ink hover:text-white hover:border-ink"
              >
                &times;
...
function FirstThenColumn({ colIdx, colName, pageIdx }: { colIdx: number; colName: string; pageIdx: number }) {
  ...
  return (
    <div className="flex flex-col border-2 border-weekly-border rounded-[10px] overflow-hidden bg-weekly-body-bg min-w-0">
      <div className="bg-weekly-head-bg border-b-2 border-b-weekly-border px-2 py-4 text-center shrink-0">
        <span style={{ fontFamily: 'Playfair Display' }} className="text-[14px] italic font-semibold text-weekly-head-text tracking-wide">{colName}</span>
      </div>
      <div
        ref={setNodeRef}
        onClick={handleMobileAddClick}
        className={`flex-1 flex items-center justify-center p-4 min-h-0 transition-colors duration-150 ${isOver ? "bg-weekly-hover" : ""} ${isMobile && !hasCard ? "cursor-pointer" : ""}`}
      >
        {hasCard ? (
          (() => {
            ...
            return (
              <div className="w-full h-full max-w-[460px] bg-white border-2 border-[#E0E5D5] rounded-[10px] flex flex-col overflow-hidden relative group">
                ...
                {isMobile && (
                  <div className="absolute top-3 left-3 w-3.5 h-3.5 bg-green-500 rounded-full border border-green-700 z-[2]" />
                )}
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    removeCard(pageIdx, String(colIdx), 0);
                  }}
    ...
    <>
      {pages.map((_, pageIdx) => (
        <div key={pageIdx} className="flex flex-col items-center gap-2">
          <div
            style={{ fontFamily: 'Playfair Display' }}
            className="text-[14px] tracking-widest uppercase text-[#B0ACA6] italic font-medium shrink-0"
          >
            Page {pageIdx + 1}
          </div>
          {scheduleType === "daily" && (
