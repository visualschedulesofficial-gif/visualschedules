...
type DFile = { id: string; item_id: string; variant: string; label: string | null; file_url: string; preview_url: string | null };
...
export default function AdminDownloadsPage() {
  ...
  const addVariantFile = async () => {
    if (!activeItem || !newVariant.trim() || !pdfFile) return;
    setBusy(true);
    ...
    await post({ kind: "file", itemId: activeItem, variant: newVariant.trim().toLowerCase(), fileUrl, previewUrl });
    setNewVariant(""); setPdfFile(null); setPreviewFile(null);
  };
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="grid md:grid-cols-3 gap-4">
          ...
          <div className="bg-card border border-border">
            <div className="px-3 py-2 border-b border-border text-[10px] uppercase tracking-wider text-ink-3">1 · Bundles</div>
            <div className="p-2 space-y-1">
              ...
              <div className="flex gap-1 pt-2">
                <input value={newBundle} onChange={(e) => setNewBundle(e.target.value)} placeholder="New bundle e.g. Morning Schedule" className="flex-1 px-2 py-1.5 border border-border text-[12px]" />
          ...
          {/* Items */}
          <div className="bg-card border border-border">
            <div className="px-3 py-2 border-b border-border text-[10px] uppercase tracking-wider text-ink-3">2 · Items {activeBundle ? "" : "(select a bundle)"}</div>
            {activeBundle && (
              <div className="p-2 space-y-1">
                {bundleItems.map((i) => (
                ...
                <div className="flex gap-1 pt-2">
                  <input value={newItem} onChange={(e) => setNewItem(e.target.value)} placeholder="New item e.g. Brushing Teeth" className="flex-1 px-2 py-1.5 border border-border text-[12px]" />
            ...
            {activeItem && (
              <div className="p-2 space-y-2">
                {itemFiles.map((f) => (
                  <div key={f.id} className="flex items-center gap-2 border border-border rounded p-1.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-[12px] text-ink capitalize">{f.variant}</div>
                ...
                <div className="border-t border-border pt-2 space-y-1.5">
                  <input value={newVariant} onChange={(e) => setNewVariant(e.target.value)} placeholder="Version name: girl / boy / mini / english / hindi" className="w-full px-2 py-1.5 border border-border text-[12px]" />
                  ...
                  <button disabled={busy || !newVariant.trim() || !pdfFile} onClick={addVariantFile} className="w-full py-1.5 bg-[#4A5A3E] text-white text-[12px] disabled:opacity-50">
        ...
        <p className="text-[11px] text-ink-3 mt-4">
          Structure: Bundle (Morning Schedule) → Items (Brushing, Bath…) → Versions (girl, boy, mini, english…). Everything appears on the public Downloads page immediately.
        </p>
      </div>
    </>
  );
