  ...
  async function handleVerifyOTP(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok && data.success) {
        setStep("done");
        setTimeout(() => { window.location.href = "/schedules"; }, 1000);
      } else {
  ...
  return (
    <div className="min-h-dvh bg-bg flex flex-col">
      <nav className="h-[56px] md:h-[66px] bg-surface border-b border-border flex items-center px-4 md:px-7 shrink-0">
        <Link href="/" className="font-serif text-base md:text-2xl italic text-ink no-underline leading-none">
          Visual Schedules
        </Link>
      </nav>
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-12">
        <div className="bg-surface w-full max-w-sm p-6 md:p-7 border border-border">
          ...
          <div className="flex gap-0 mb-5 border border-border">
            <button
              onClick={() => { setMode("user"); setError(""); setStep("email"); }}
            ...
            <button
              onClick={() => { setMode("admin"); setError(""); setEmail("admin@dataorc.in"); }}
          ...
          {mode === "user" && (
            <>
              ...
              <p className="text-[13px] text-ink-2 leading-relaxed mb-5">
                {step === "email" && "Enter your email to receive a 6-digit sign-in code."}
                {step === "otp" && `We sent a code to ${email}. Enter it below.`}
                {step === "done" && "You're signed in! Redirecting..."}
              </p>
              ...
              {step === "otp" && (
                <form onSubmit={handleVerifyOTP}>
                  <label className="text-[11px] tracking-widest uppercase text-[#8A8480] mb-1.5 block font-medium">6-Digit Code</label>
                  <input
                    type="text"
                    value={otp}
                    ...
                    maxLength={6}
                    autoFocus
                    className="w-full py-3 px-3 border border-border bg-surface-hover font-sans text-[18px] text-ink text-center tracking-[6px] outline-none focus:border-accent mb-4 font-medium"
                  />
                  {error && <p className="text-xs text-[#C53030] mb-3">{error}</p>}
                  <button
                    type="submit"
              ...
              {step === "done" && (
                <div className="text-center py-4">
                  ...
                  <p className="text-sm text-ink font-medium">Signed in! Redirecting...</p>
                </div>
              )}
            </>
          )}
          ...
          {mode === "admin" && (
            <>
              <h1 className="font-serif text-xl italic text-ink mb-1.5">Admin Login</h1>
              <p className="text-[13px] text-ink-2 leading-relaxed mb-5">
          ...
          <div className="mt-5 pt-4 border-t border-border text-center">
            <p className="text-xs text-ink-3">
              No account needed for free cards. Sign in to unlock paid categories.
            </p>
          </div>
        </div>
      </main>
